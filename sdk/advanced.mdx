...
const result = await client.transaction(async (tx) => {
  ...
  const order = await tx.sap.A_SalesOrder.create({
    SalesOrderType: 'OR',
    SoldToParty: '10100001'
  });

  // Create order items
  await tx.sap.A_SalesOrderItem.create({
...
// Queue operations
batch.add(client.sap.A_BusinessPartner.list({ top: 10 }));
batch.add(client.sap.A_SalesOrder.list({ top: 10 }));
batch.add(client.sap.A_Product.get('TG11'));
...
const availability = await client.sap.A_Product
...
await client.sap.A_SalesOrder
  .boundAction('1000000', 'Release');

// Approve with parameters
await client.sap.A_PurchaseOrder
...
```typescript
const response = await client.sap.A_BusinessPartner.list({
  raw: true
});
...
await client.sap.A_BusinessPartner.update('10100001', {
...
```typescript
const order = await client.sap.A_SalesOrder.createDeep({
  SalesOrderType: 'OR',
  SoldToParty: '10100001',
...
```typescript
const stream = client.sap.A_BusinessPartner.stream({
