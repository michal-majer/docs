...
```typescript
const results = await client.sap.A_BusinessPartner.list({
...
// Not equal
const nonCustomers = await client.sap.A_BusinessPartner.list({
  filter: "BusinessPartnerCategory ne '1'"
});

// Numeric comparison
const largeOrders = await client.sap.A_SalesOrder.list({
  filter: "TotalNetAmount gt 10000"
});

// Date comparison
const recentOrders = await client.sap.A_SalesOrder.list({
...
```typescript
// Contains
const searchResults = await client.sap.A_BusinessPartner.list({
  filter: "substringof('Corp', BusinessPartnerName)"
});

// Starts with
const aPartners = await client.sap.A_BusinessPartner.list({
  filter: "startswith(BusinessPartnerName, 'A')"
});

// Ends with
const ltdCompanies = await client.sap.A_BusinessPartner.list({
...
```typescript
// AND
const activeCustomers = await client.sap.A_BusinessPartner.list({
  filter: "BusinessPartnerCategory eq '1' and IsActiveEntity eq true"
});

// OR
const customersOrSuppliers = await client.sap.A_BusinessPartner.list({
  filter: "BusinessPartnerCategory eq '1' or BusinessPartnerCategory eq '2'"
});

// NOT
const notDeleted = await client.sap.A_BusinessPartner.list({
  filter: "not(IsMarkedForDeletion)"
});

// Complex combinations
const complexFilter = await client.sap.A_BusinessPartner.list({
...
```typescript
// Check for null
const withoutIndustry = await client.sap.A_BusinessPartner.list({
  filter: "Industry eq null"
});

// Check not null
const withIndustry = await client.sap.A_BusinessPartner.list({
...
const sorted = await client.sap.A_BusinessPartner.list({
  orderBy: 'BusinessPartnerName'
});

// Single field, descending
const sortedDesc = await client.sap.A_BusinessPartner.list({
  orderBy: 'BusinessPartnerName desc'
});

// Multiple fields
const multiSorted = await client.sap.A_SalesOrder.list({
...
// First page
const page1 = await client.sap.A_BusinessPartner.list({
  top: pageSize,
  skip: 0
});

// Second page
const page2 = await client.sap.A_BusinessPartner.list({
  top: pageSize,
  skip: pageSize
});
```
...
for await (const partners of client.sap.A_BusinessPartner.paginate({ pageSize: 100 })) {
...
```typescript
// Single expand
const partnersWithAddresses = await client.sap.A_BusinessPartner.list({
  expand: ['to_BusinessPartnerAddress']
});

// Multiple expands
const ordersWithDetails = await client.sap.A_SalesOrder.list({
  expand: ['to_Item', 'to_Partner']
});

// Nested expand (OData v4)
const deepExpand = await client.sap.A_SalesOrder.list({
...
```typescript
const results = await client.sap.A_BusinessPartner
  .query()
  .select('BusinessPartner', 'BusinessPartnerName', 'Industry')
  .filter('BusinessPartnerCategory', 'eq', '1')
...
const ordersInRange = await client.sap.A_SalesOrder.list({
...
```typescript
const specificPartners = await client.sap.A_BusinessPartner.list({
